...
def metricas_pronostico(y_real, y_pred) -> dict:
    ...
    return {
        "Error_medio_pronostico": float((y_real - y_pred).mean()),
        ...
        "interpretacion_error_medio": ("Subestima en promedio" if float((y_real - y_pred).mean()) > 0 else ("Sobrestima en promedio" if float((y_real - y_pred).mean()) < 0 else "Sin sesgo promedio observable")),
