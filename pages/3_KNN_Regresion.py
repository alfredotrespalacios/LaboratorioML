...
def preparar_datos_knn(df_base: pd.DataFrame):
...
def crear_tabla_doble_escala(df_pred: pd.DataFrame, usar_log: bool) -> pd.DataFrame:
    work = df_pred.copy()
    if usar_log:
        work["y_real_nivel"] = np.exp(work["y_real"])
        pred_col = "y_predicho" if "y_predicho" in work.columns else "y_pronosticada"
        work[f"{pred_col}_nivel"] = np.exp(work[pred_col])
        if "residual" in work.columns:
            work["residual_nivel"] = work["y_real_nivel"] - work[f"{pred_col}_nivel"]
        if "error_pronostico" in work.columns:
...
if st.button("Entrenar KNN Regresión"):
    ...
    pred_df = crear_tabla_doble_escala(pred_df, usar_log_y)
    ...
    if not fore_df.empty:
        ...
        pronostico_df = crear_tabla_doble_escala(pronostico_df, usar_log_y)
    ...
    metricas_fore_nivel = metricas_pronostico(pronostico_df["y_real_nivel"], pronostico_df["y_pronosticada_nivel"]) if usar_log_y and modo_pronostico == MODO_EVALUACION and not pronostico_df.empty and "y_real_nivel" in pronostico_df.columns and "y_pronosticada_nivel" in pronostico_df.columns else {}
    ...
    if not pronostico_df.empty:
        tmp = pd.DataFrame({"eje_x": pronostico_df["eje_x"], "y_observada": pronostico_df["y_real"] if "y_real" in pronostico_df.columns else np.nan, "y_estimada": np.nan, "y_pronosticada": pronostico_df["y_pronosticada"]})
    ...
    if modo_pronostico == MODO_EVALUACION and not fore_df.empty:
        vals = []
        k_lim = int(min(k_max_grafico, len(train_df)))
        for kval in range(1, k_lim + 1):
            _, _, _, _, yk = fit_predict_knn(X_train, y_train, fore_df[x_nombres], kval, estandarizar_x)
            mf = metricas_pronostico(fore_df[y_nombre].values, yk)
            vals.append({"K": kval, **mf})
    ...
    try:
        ...
        muestra_model = scaler.transform(muestra_objetivo) if estandarizar_x and scaler is not None else muestra_objetivo.values
    ...
    st.session_state["resultados_knn_regresion_actuales"] = {
        ...
        **info_modo,
        ...
        "vecinos_ejemplo": vecinos_df.to_dict(orient="records"),
    }
...
if res:
    ...
    vecinos_df = pd.DataFrame(res.get("vecinos_ejemplo", []))
    ...
    with c4:
        ...
        pdf_bytes = crear_pdf_modulo("Informe del módulo: KNN Regresión", res, tablas=[("Descripción de variables", desc_df), ("Configuración", pd.DataFrame([{"K": res.get("K"), "distancia": res.get("distancia"), "ponderacion": res.get("ponderacion"), "estandarizacion_x": res.get("estandarizacion_x"), "transformacion_y": res.get("transformacion_y")} ])), ("Modo de pronóstico", pd.DataFrame([{"modo_pronostico": res.get("modo_pronostico"), "uso_modo_pronostico": res.get("uso_modo_pronostico"), "implicacion_modo_pronostico": res.get("implicacion_modo_pronostico")} ])), ("Métricas", metricas_df), ("Métricas de pronóstico", metricas_pronostico_df), ("Métricas de pronóstico en nivel", metricas_pronostico_nivel_df), ("Predicciones", pred_df), ("Pronóstico", pronostico_df), ("Error por K", error_k_df), ("Vecinos ejemplo", vecinos_df)], figuras=figs, notas=["KNN predice por similitud: usa el promedio simple de los valores Y de los K vecinos más cercanos.", "La estandarización es importante porque las distancias dependen de la escala de las variables X.", res.get("uso_modo_pronostico", ""), res.get("implicacion_modo_pronostico", "")])
