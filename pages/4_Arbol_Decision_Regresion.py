...
def extraer_reglas(modelo, feature_names):
    tree_text = export_text(modelo, feature_names=list(feature_names), decimals=4)
    lineas = [x for x in tree_text.splitlines() if "value:" in x]
    rows = []
    for i, line in enumerate(lineas, start=1):
        rows.append({"regla_id": i, "regla_texto": line.strip()})
...
if res:
    ...
    with st.expander("Reglas principales del árbol", expanded=True):
        ...
        st.dataframe(reglas_mostrar_df, use_container_width=True)
    ...
    with c4:
        ...
        pdf_bytes = crear_pdf_modulo(
            ...
            figuras=figs,
            notas=["El árbol divide los datos mediante reglas sucesivas. En cada hoja predice el promedio de Y.", "La figura del árbol se limita visualmente para mantener legibilidad.", res.get("uso_modo_pronostico", ""), res.get("implicacion_modo_pronostico", "")]
        )
