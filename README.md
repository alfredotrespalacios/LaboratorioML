# Laboratorio de Machine Learning e IA para Finanzas - versión completa v1.17
...
- Se agrega el módulo independiente `3_KNN_Regresion.py` sin renombrar los módulos existentes para evitar errores de navegación si quedan archivos antiguos en Streamlit Cloud.
- Se agrega `data/datos_knn_regresion.xlsx`, copia pedagógica de los datos de regresión lineal para facilitar comparación.
- KNN Regresión permite transformar Y y X con logaritmos, reportar resultados en ln(Y) y en nivel original cuando aplica, escoger K, estandarizar X, usar modo de pronóstico, ver error por K y revisar vecinos más cercanos.
- Se elimina KNN Regressor del módulo de Otras regresiones para evitar duplicidad.
- Se agrega U de Theil al módulo de regresión lineal.
- Resumen Ejecutivo incluye KNN Regresión.


## Cambios v1.15

- Reorganización limpia a 10 módulos:
  1. Estadística descriptiva
  2. Regresión lineal
  3. KNN Regresión
...
- Se agrega el módulo independiente **Árbol de decisión - Regresión**.
- Se mantiene KNN como módulo independiente y se mejora su explicación pedagógica.
- Se retiran KNN y árbol de decisión del módulo **Otras regresiones** para evitar duplicidades.
- **Otras regresiones** queda con Random Forest, Gradient Boosting, XGBoost y Support Vector Regression.
- Se agrega `xgboost` a `requirements.txt`.
- Se usan archivos de datos separados para estadística descriptiva, KNN y árbol, con la misma base pedagógica de regresión lineal.
- Se agrega descripción de variables como componente común, visible en un expander abierto por defecto.
- El resumen ejecutivo incluye KNN y Árbol de decisión, y solicita una sección “Principios de los modelos utilizados”.
- Se agregan y refuerzan métricas como Error medio de pronóstico y U de Theil.
- En árbol de decisión se incluye figura del árbol, importancia de variables y reglas principales.
- En KNN se mejora la sección de vecinos para aclarar cuál observación sirve como referencia.


## Cambios v1.16

- Se corrige el error de importación del módulo `3_KNN_Regresion.py`.
- La página KNN intentaba importar `interpretar_u_theil`, pero la función disponible en `utils/regresion_helpers.py` se llama `interpretacion_u_theil`.
- Se elimina el import innecesario para que el módulo KNN cargue correctamente en Streamlit Cloud.


## Cambios v1.17

- En el módulo `Árbol de decisión - Regresión` se corrige la tabla de reglas.
- Ahora se muestra la regla completa en la columna `regla_texto`, no solo el `regla_id`.
- Cada regla corresponde a un camino completo desde la raíz hasta una hoja del árbol.
- La tabla incluye:
  - `regla_id`
  - `regla_texto`
  - `valor_estimado`
  - `n_observaciones_en_hoja`
- Las reglas completas se mantienen en pantalla, Excel, PDF, JSON y resultados guardados para el informe ejecutivo.
